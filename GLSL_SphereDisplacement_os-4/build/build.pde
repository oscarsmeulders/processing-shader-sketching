...
void setup() {
  ...
  ap = minim.loadFile("../../_muzak/ballroom.mp3", 2048);
  ap.play();
...
void draw() {
  ...
  for (int i = 10; i <= 10; i++) {
    if ( bd.isOnset( i ) ) {
      //println("db is waar 10");
      rotationYExtra = rotationYExtra+50;
    }
  }
  ...
  for (int i = 11; i <= 15; i++) {
    if ( bd.isOnset( i ) ) {
      //println("db is waar 10");
      rotationZExtra = rotationZExtra+10;
    }
  }
  ...
  rotateY(  (frameCount*-0.04) + radians(rotationYExtra) );
  rotateZ(  (frameCount*-0.01) + radians(rotationZExtra) );
